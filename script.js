...
function toast(msg, type='info') {
  ...
  setTimeout(() => { el.style.cssText='opacity:0;transform:translateX(20px);transition:all .3s';
...
function renderDash() {
  ...
  if(!pp.length){ c.innerHTML=`<div class="empty"><div class="empty-ico">🏭</div><div class="empty-title">Sin productos aún</div><div class="empty-sub">Registra tu primer producto para comenzar</div></div>`; return; }
  c.innerHTML=`<div class="tbl-wrap"><table>
    ...
    <tbody>${pp.map(p=>{ const nm=S.movs.filter(m=>m.cod===p.cod).length; const sc=p.stk<=0?'zero':p.stk<5?'low':'ok'; return `<tr>
...
function limpProd() {
  ['p-cod','p-nom','p-stk','p-cu'].forEach(id=>document.getElementById(id).value='');
...
function renderCatalog() {
  ...
  if(!pp.length){ g.innerHTML=`<div class="empty"><div class="empty-ico">📦</div><div class="empty-title">Sin productos</div><div class="empty-sub">Registra tu primer producto arriba</div></div>`; return; }
  g.innerHTML=`<div class="prod-grid">${pp.map(p=>{ const sc=p.stk<=0?'zero':p.stk<5?'low':'ok'; const sl=p.stk<=0?'Sin stock':p.stk<5?'Stock bajo':`${p.stk} uds.`; const nm=S.movs.filter(m=>m.cod===p.cod).length; return `<div class="prod-card">
      <div class="pc-head"><span class="pc-code">${p.cod}</span><span class="pc-stk ${sc}">${sl}</span></div>
...
function setTipo(t) {
  ...
  if(t==='ingreso'){ b.className='btn btn-s'; b.textContent='📥 Registrar Ingreso'; }
  else             { b.className='btn btn-d'; b.textContent='📤 Registrar Salida'; }
  prevMov();
}

function fillProdSel() {
  const sel=document.getElementById('m-prod'), cur=sel.value;
  sel.innerHTML='<option value="">— Selecciona un producto —</option>'+ 
    Object.values(S.prods).map(p=>`<option value="${p.cod}">${p.cod} — ${p.nom}</option>`).join('');
...
function prevMov() {
  ...
  const p=S.prods[cod], t=S.tipo;
  let ns,np,nt;
  if(t==='ingreso'){ ns=p.stk+qty; nt=p.ct+(qty*cu); np=ns>0?nt/ns:cu; b.className='prev-box show'; }
  else             { if(qty>p.stk){ b.classList.remove('show'); return; } ns=p.stk-qty; np=p.cp; nt=ns*np; b.className='prev-box r show'; }
  const cl=t==='ingreso'?'g':'r';
  ['pm-sl','pm-pl','pm-tl'].forEach(id=>{ document.getElementById(id).className=`prev-lbl ${cl}`; });
...
function regMov() {
  ...
  if(t==='ingreso') {
    ...
    const ns=p.stk+qty, nt=p.ct+(qty*cu), np=ns>0?nt/ns:cu;
    S.movs.push({id:S.nid++,cod,nom:p.nom,t:'ingreso',qty,cu,cmt:qty*cu,sa:p.stk,pa:p.cp,sd:ns,pd:np,td:nt,ref,fecha:new Date().toLocaleString('es-PE')});
    S.prods[cod]={...p,stk:ns,ct:nt,cp:np};
    toast(`+${qty} uds. ingresadas a "${p.nom}" ✓`,'ok');
  } else {
    if(qty>p.stk) return toast('Stock insuficiente','err');
    const ns=p.stk-qty, nt=ns*p.cp;
    S.movs.push({id:S.nid++,cod,nom:p.nom,t:'salida',qty,cu:p.cp,cmt:qty*p.cp,sa:p.stk,pa:p.cp,sd:ns,pd:p.cp,td:nt,ref,fecha:new Date().toLocaleString('es-PE')});
    S.prods[cod]={...p,stk:ns,ct:nt};
    toast(`-${qty} uds. despachadas de "${p.nom}" ✓`,'ok');
  }
  limpMov(); refresh(); onProd();
}

function limpMov() {
  ['m-qty','m-cu','m-ref'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('pmov').classList.remove('show');
}

/* ── HISTORIAL ─────────────────────────────── */
function fillFiltros() {
  const sel=document.getElementById('f-prod'),cur=sel.value;
...
function renderHist() {
  ...
  if(!lista.length){ w.innerHTML=`<div class="empty"><div class="empty-ico">📋</div><div class="empty-title">Sin movimientos</div><div class="empty-sub">Aparecerán aquí cuando registres operaciones</div></div>`; return; }
  w.innerHTML=`<div class="tbl-wrap"><table>
    <thead><tr><th>#</th><th>Fecha</th><th>Producto</th><th>Tipo</th><th>Cant.</th><th>C.Unit.</th><th>C.Mov.</th><th>Stock</th><th>C.Promedio</th><th>Ref.</th></tr></thead>
    <tbody>${lista.map(m=>`<tr>
      ...
      <td><span class="td-code">${m.cod}</span><div style="font-size:12px;color:var(--c-muted);margin-top:2px">${m.nom}</div></td>
      <td><span class="badge badge-${m.t}">${m.t==='ingreso'?'▲ Ingreso':'▼ Salida'}</span></td>
      <td class="td-mono"><strong style="color:${m.t==='ingreso'?'var(--c-green)':'var(--c-red)'}">${m.t==='ingreso'?'+':'-'}${m.qty}</strong></td>
      <td class="td-mono">${fmt(m.cu)}</td>
      <td class="td-mono"><strong>${fmt(m.cmt)}</strong></td>
      <td><div class="arr"><span class="arr-from">${m.sa}</span><span class="arr-ic">→</span><span class="arr-to">${m.sd}</span></div></td>
      <td><div class="arr"><span class="arr-from">${fmtN(m.pa)}</span><span class="arr-ic">→</span><span class="arr-to">${fmtN(m.pd)}</span></div></td>
...
function exportCSV() {
  ...
  const rows=[['ID','Fecha','Codigo','Producto','Tipo','Cantidad','CostoUnit','CostoMov','StockAntes','StockDespues','PromAntes','PromDespues','Referencia'],
    ...S.movs.map(m=>[m.id,m.fecha,m.cod,m.nom,m.t,m.qty,fmtN(m.cu),fmtN(m.cmt),m.sa,m.sd,fmtN(m.pa),fmtN(m.pd),m.ref||''])];
  const csv=rows.map(r=>r.map(c=>`"${c}"`).join(',')).join('\n');
  const a=Object.assign(document.createElement('a'),{href:URL.createObjectURL(new Blob([csv],{type:'text/csv'})),download:`almacen_${Date.now()}.csv`});
